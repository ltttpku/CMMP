...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, adapter: bool=False, adapter_num_layers: int=1, bottleneck: int=64):
        super().__init__()
        ...
        if adapter:
            self.adaptermlp = Adapter(None,  d_model=d_model , dropout=0.1, bottleneck=bottleneck,
                                    init_option='lora',
    ...
    def forward(self, x: torch.Tensor):
        ...
        '''
        x, prior = x  
        if self.adapter and prior is not None:
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, adapter: bool=False, adapter_layers: List=[i for i in range(24)], adapter_num_layers: int=1, bottleneck: int=64):
        super().__init__()
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, adapter=((_ in adapter_layers) and adapter), adapter_num_layers=adapter_num_layers, bottleneck=bottleneck) for _ in range(layers)])

    def forward(self, x: torch.Tensor, prior=None):
        return self.resblocks((x,prior))[0]

class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, use_adapter: bool=True, adapter_layers: List=[_ for _ in range(24)], adapter_num_layers: int=1, bottleneck: int=64):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.transformer = Transformer(width, layers, heads, adapter=use_adapter, adapter_layers=adapter_layers, adapter_num_layers=adapter_num_layers, bottleneck=bottleneck)
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer(
                ...
                heads=vision_heads,
                output_dim=embed_dim,
                use_adapter=kwargs["use_adapter"],
        ...
        self.transformer = Transformer(
            ...
            heads=transformer_heads,
            attn_mask=self.build_attention_mask(),
            adapter=kwargs["use_text_adapter"],
            adapter_layers=kwargs["adapter_layers"],
            adapter_num_layers=kwargs["adapter_num_layers"],
        )
...
def build_model(state_dict: dict, use_adapter=True, adapter_pos='all', adapter_num_layers=1, use_text_adapter=False, bottleneck=64):
    ...
    model = CLIP(
        ...
        use_text_adapter=use_text_adapter, bottleneck=bottleneck,
    )
